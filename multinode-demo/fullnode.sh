...
[[ -r "$fullnode_id_path" ]] || solana-keygen -o "$fullnode_id_path"
[[ -r "$fullnode_vote_id_path" ]] || solana-keygen -o "$fullnode_vote_id_path"

fullnode_id=$(solana-keygen pubkey "$fullnode_id_path")
fullnode_vote_id=$(solana-keygen pubkey "$fullnode_vote_id_path")
...
airdrop() {
  ...
  declare address
  address=$(solana-wallet --keypair "$keypair_file" address)
  ...
  while ! solana-wallet --keypair "$keypair_file" --host "$host" airdrop "$amount"; do
...
setup_vote_account() {
  ...
  declare node_id
  node_id=$(solana-wallet --keypair "$node_id_path" address)

  declare vote_id
  vote_id=$(solana-wallet --keypair "$vote_id_path" address)
  ...
  if [[ -f "$vote_id_path".configured ]]; then
    ...
    solana-wallet --keypair "$node_id_path" --host "$drone_address" \
      create-vote-account "$vote_id" "$node_id" $((stake - 1)) || return $?

    touch "$vote_id_path".configured
  fi

  solana-wallet --keypair "$node_id_path" --host "$drone_address" show-vote-account "$vote_id"
  return 0
}
...
while true; do
  ...
  if [[ ! -d "$ledger_config_dir" ]]; then
    ...
    solana-ledger-tool --ledger "$ledger_config_dir" verify
  fi
